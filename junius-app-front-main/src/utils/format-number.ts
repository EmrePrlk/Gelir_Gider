...
function getLocaleCode() {
  ...
  const code = currentLang?.numberFormat?.code;
  const currency = currentLang?.numberFormat?.currency;
...
export function fShortenNumber(inputValue: InputValue) {
  ...
  return fm.replaceAll(/[A-Z]/g, match => match.toLowerCase());
}
...
export function fData(inputValue: InputValue) {
  ...
  const fm = `${Number.parseFloat(
    (number / baseValue ** index).toFixed(decimal),
  )} ${units[index]}`;
